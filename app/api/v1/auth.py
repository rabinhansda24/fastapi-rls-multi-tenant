...
async def register_new_tenant(request: TenantRegistrationRequest, db=Depends(get_db)):
    ...
    try:
        logger.info("Tenant registration requested tenant_name=%s admin_email=%s", request.name, request.admin_email)
        return register_tenant(request, db=db)
    except Exception as e:
        logger.exception("Tenant registration failed tenant_name=%s admin_email=%s", request.name, request.admin_email)
...
async def login(tenant_slug: str, trequest: LoginRequest, db=Depends(get_db)):
    ...
    if not tenant:
        logger.warning("Login forbidden for unknown tenant slug=%s email=%s", tenant_slug, trequest.email)
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Request Forbidden")
    try:
        logger.info("Login requested tenant_slug=%s email=%s", tenant_slug, trequest.email)
        return login_tenant(db, trequest=trequest, tenant=tenant)
    except Exception as e:
        logger.exception("Login failed tenant_slug=%s email=%s", tenant_slug, trequest.email)
        raise HTTPException(status_code=400, detail=str(e))
